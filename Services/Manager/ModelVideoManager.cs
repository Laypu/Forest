...
{
    public class ModelVideoManager : IModelVideoManager
    {
        readonly SQLRepository<ModelVideoMain> _sqlrepository;
        readonly SQLRepository<GroupVideo> _groupsqlrepository;
        readonly SQLRepository<VideoItem> _sqlitemrepository;
        readonly SQLRepository<VideoUnitSetting> _unitsqlitemrepository;
        ...
        public ModelVideoManager(SQLRepositoryInstances sqlinstance)
        {
            _sqlrepository = sqlinstance.ModelVideoMain;
            _groupsqlrepository = sqlinstance.GroupVideo;
            _seosqlrepository = sqlinstance.SEO;
            _sqlitemrepository = sqlinstance.VideoItem;
            _unitsqlitemrepository = sqlinstance.VideoUnitSetting;
        ...
        #region GetAll
        public IEnumerable<ModelVideoMain> GetAll()
        {
        ...
        #region Where
        public IEnumerable<ModelVideoMain> Where(ModelVideoMain model)
        {
        ...
        public string UpdateSeq(int id, int seq, string langid, string account, string username)
        {
            try
            {
                ...
                {
                    IList<ModelVideoMain> mtseqdata = null;
                    ...
                    IList<ModelVideoMain> ltseqdata = null;
            ...
            {
                NLogManagement.SystemLogInfo("更新ModelVideoMain排序失敗:" + " error:" + ex.Message);
                return "更新ModelVideoMain排序失敗:" + " error:" + ex.Message;
            }
        }
        #endregion
        ...
        public string UpdateGroupSeq(int id, int seq, string mainid, string account, string username)
        {
            try
            {
                ...
                if (oldmodel.First().Sort != seq)
                {
                    IList<GroupVideo> mtseqdata = null;
                    ...
                    var qseq = (seq > oldmodel.First().Sort) ? seq : oldmodel.First().Sort;
                    IList<GroupVideo> ltseqdata = null;
        ...
        public string Delete(string[] idlist, string delaccount, string langid, string account, string username)
        {
            try
            {
                ...
                {
                    var m = _menusqlrepository.GetByWhere("ModelID=18 and ModelItemID=@1", new object[] { idlist[idx] });
                    if (m.Count() > 0)
                    {
                        return "刪除失敗,刪除的項目使用中..";
                    }
                    var p = _pagesqlrepository.GetByWhere("ModelID=18 and ModelItemID=@1", new object[] { idlist[idx] });
                    if (p.Count() > 0)
                    {
                        return "刪除失敗,刪除的項目使用中..";
                    }
                ...
                for (var idx = 0; idx < idlist.Length; idx++)
                {
                    ...
                    if (r <= 0)
                    {
                        ...
                        _seosqlrepository.DelDataUseWhere("TypeName='Video' and TypeID=@1", new object[] { idlist[idx] });
                        ...
                        foreach (var items in olditem)
                        {
                            ...
                            _seosqlrepository.DelDataUseWhere("TypeName='VideoItem' and TypeID=@1", new object[] { items.ItemID });
        ...
        #region PagingGroup
        public Paging<GroupVideo> PagingGroup(SearchModelBase model)
        {
            var Paging = new Paging<GroupVideo>();
            var onepagecnt = model.Limit;
            var whereobj = new List<string>();
            var wherestr = new List<string>();
        ...
        public string UpdateGroupStatus(string id, bool status, string account, string username)
        {
            try
            {
                ...
                if (r >= 0)
                {
                    NLogManagement.SystemLogInfo("修改GroupVideo顯示狀態 id=" + id + " 為:" + status);
                    return "更新成功";
                }
                else
                {
            ...
            {
                NLogManagement.SystemLogInfo("修改GroupVideo顯示狀態失敗:" + ex.Message);
                return "更新失敗";
            }
        }
        #endregion
        ...
        #region GetUnitModel
        public VideoUnitSettingModel GetUnitModel(string modelid)
        {
            var data = _unitsqlitemrepository.GetByWhere("MainID=@1", new object[] { modelid });
            var model = new VideoUnitSettingModel();
            model.MainID = int.Parse(modelid);
            if (data.Count() > 0)
            {
                model = new VideoUnitSettingModel()
                {
            ...
            if (model.UnitSettingColumnList.Count() == 0) {
                var columnlist = _columnsqlrepository.GetByWhere("Type='Video'", null).OrderBy(v=>v.Sort);
        ...
        public string UpdateColumnSeq(int id, int seq, string account, string username)
        {
            try
            {
                ...
                if (oldmodel.First().Sort != seq)
                {
                    ...
                    mtseqdata = _columnsqlrepository.GetByWhere("Sort>@1 and Type=@2", new object[] { seq, "Video" }).OrderBy(v => v.Sort).ToArray();

                    var updatetime = DateTime.Now;
                    if (mtseqdata.Count() == 0)
                    {
                        var totalcnt = 0;
                        totalcnt = _columnsqlrepository.GetCountUseWhere("Type=@1", new object[] { "Video" });
                        seq = totalcnt;
                    }

                    var qseq = (seq > oldmodel.First().Sort) ? seq : oldmodel.First().Sort;
                    IList<ColumnSetting> ltseqdata = null;
                    ltseqdata = _columnsqlrepository.GetByWhere("Sort<=@1  and Type=@2", new object[] { qseq, "Video" }).OrderBy(v => v.Sort).ToArray();
                    //更新seq+1
                    var sidx = 0;
                    for (var idx = 1; idx <= ltseqdata.Count(); idx++)
                    {
        ...
        #region SetUnitModel
        public string SetUnitModel(VideoUnitSettingModel model, string account)
        {
            var newmodel = new VideoUnitSetting();
        ...
        {
            var seodata = _seosqlrepository.GetByWhere("TypeName=@1 and TypeID=@2", new object[] { "Video", modelid });
        ...
        #region CreateItem
        public string CreateItem(VideoEditModel model, string LangId, string account)
        {
            ...
            if (r > 0)
            {
                _verifydatasqlrepository.DelDataUseWhere("ModelID=@1 and ModelMainID=@2 and ModelItemID=@3", new object[] { 18, savemodel.ModelID.Value,savemodel.ItemID });
        ...
        public VideoEditModel GetModelByID(string modelid, string itemid)
        {
            ...
            if (data.Count() > 0)
            {
                ...
                var seodata = _seosqlrepository.GetByWhere("TypeName='VideoItem' and TypeID=@1", new object[] { itemid });
                var model = new VideoEditModel()
                {
                    ...
                    ImageUrl = VirtualPathUtility.ToAbsolute("~/UploadImage/VideoItem/" + fdata.ImageFileName),
                    ...
                    Title = fdata.Title,
                     RelateImageFileOrgName = fdata.RelateImageFileOrgName,
                      RelateImageName = fdata.RelateImageFileName,
                        RelateImagelUrl= VirtualPathUtility.ToAbsolute("~/UploadImage/VideoItem/" + fdata.RelateImageFileName),
        ...
        #region UpdateItem
        public string UpdateItem(VideoEditModel model, string LangId, string account)
        {
            var iswriteseo = false;
            if (model.WebsiteTitle.IsNullorEmpty() == false || model.Description.IsNullorEmpty() == false
                   || (model.Keywords!=null && model.Keywords.Any(v => v.IsNullorEmpty() == false)))
            ...
            _seosqlrepository.DelDataUseWhere("TypeName='VideoItem' and TypeID=@1", new object[] { model.ItemID });
            var admin = _Usersqlrepository.GetByWhere("Account=@1", new object[] { account });
            var savemodel = new VideoItem()
            {
                ItemID= model.ItemID,
            ...
            {
                var hasvdata = _verifydatasqlrepository.GetByWhere("ModelID=@1 and ModelMainID=@2 and ModelItemID=@3", new object[] {
                     18, savemodel.ModelID.Value,savemodel.ItemID
                });
                if (hasvdata.Count() == 0)
                {
                    _verifydatasqlrepository.Create(new VerifyData()
                ...
                else
                {
                    _verifydatasqlrepository.Update("VerifyStatus=0,ModelStatus=2,VerifyDateTime=Null,VerifyUser='',VerifyName='',ModelName=@1,UpdateDateTime=@2,UpdateUser=@3,UpdateAccount=@4", 
                        "ModelID=@5 and ModelMainID=@6 and ModelItemID=@7", new object[] {
                           savemodel.Title,DateTime.Now, (admin.Count() == 0 ? "" : admin.First().User_Name),account,18 , savemodel.ModelID.Value, savemodel.ItemID
                    });
                }
        ...
        #region PagingItem
        public Paging<VideoItemResult> PagingItem(string modelid, VideoSearchModel model)
        {
            ...
            var modelverifydata = _verifydatasqlrepository.GetByWhere("ModelID=18 and ModelMainID=@1", new object[] { modelid });
        ...
        public string UpdateItemSeq(int modelid,int id, int seq,  string account, string username)
        {
            try
            {
                ...
                if (oldmodel.First().Sort != seq)
                {
                    IList<VideoItem> mtseqdata = null;
                    ...
                    var qseq = (seq > oldmodel.First().Sort) ? seq : oldmodel.First().Sort;
                    IList<VideoItem> ltseqdata = null;
            ...
            {
                NLogManagement.SystemLogInfo("更新VideoItem排序失敗:" + " error:" + ex.Message);
                return "更新VideoItem排序失敗:" + " error:" + ex.Message;
            }
        }
        #endregion
        ...
        public string SetItemStatus(string id, bool status, string account, string username)
        {
            try
            {
                ...
                if (r >= 0)
                {
                    NLogManagement.SystemLogInfo("修改VideoItem顯示狀態 id=" + id + " 為:" + status);
                    return "更新成功";
                }
                else
                {
            ...
            {
                NLogManagement.SystemLogInfo("修改VideoItem顯示狀態失敗:" + ex.Message);
                return "更新失敗";
            }
        }
        #endregion
            ...
            try
            {
                ...
                for (var idx = 0; idx < idlist.Length; idx++)
                {
                    ...
                    if (r <= 0)
                    {
                        ...
                        _verifydatasqlrepository.DelDataUseWhere("ModelID=18 and ModelMainID=@1 and ModelItemID=@2", new object[] { modelid, idlist[idx] });

                        _seosqlrepository.DelDataUseWhere("TypeName='VideoItem' and TypeID=@1", new object[] { idlist[idx] });
        ...
        public Paging<VideoItemResult> PagingItemForWebSite(string modelid, VideoSearchModel model,string nogroupstr)
        {
            var Paging = new Paging<VideoItemResult>();
            var data = new List<VideoItem>();
        ...
        #region GetModelItem
        public VideoItem GetModelItem(string itemid)
        {
            var data = _sqlitemrepository.GetByWhere("ItemID=@1", new object[] { itemid });
            if (data.Count() > 0)
            {
        ...
        public void UpdateClickCount(string itemid)
        {
            ...
            _clicktablesqlitemrepository.DelDataUseWhere("Type='Video' and LastClickDatetime<@1", new object[] { DateTime.Now.AddDays(-1) });
            var hasdata = _clicktablesqlitemrepository.GetByWhere("Type='Video' and IP=@1 and ID=@2", new object[] { tClientIP, itemid });
            if (hasdata.Count() == 0)
            {
        ...
        public SyndicationFeed GetSyndicationFeedData(string itemid,string menuid)
        {
            ...
            foreach (var d in data)
            {
                ...
                {
                    url = helper.Action("VideoView",new { id= d.ItemID });
                }
                else {
                    url = helper.Action("VideoView", new { id = d.ItemID, mid = menuid });
                }
        ...
        public string[] GetVideoMore(string mainID,string nowid,string menuid)
        {
            var idx = 1;
            var whereobj = new List<object>();
            var wherestr = new List<string>();
            ...
            whereobj.Add(nowdate);
            var str = string.Join(" and ", wherestr);
            var videoitem = _sqlitemrepository.GetByWhere(str + " order by Sort", whereobj.ToArray()).ToList();
            var sb = new System.Text.StringBuilder();
        
            UrlHelper helper = new UrlHelper(HttpContext.Current.Request.RequestContext);
            var baseimg = helper.Content("~/ContentWebsite/image/logo_400x300.jpg");

            foreach (var v in videoitem) {
                sb.Append("<div class='item'>");
                if (v.Link_Mode == 1)
                {
                    if (menuid != "-1")
                    {
                        sb.Append("<a href='" + helper.Action("VideoView", new { id = v.ItemID, mid = menuid,  groupid = v.GroupID }) + "' title='" + v.Title + "'>");
                    }
                    else
                    {
                        sb.Append("<a href='" + helper.Action("VideoView", new { id = v.ItemID,groupid = v.GroupID }) + "' title='" + v.Title + "'> ");
                    }
                }
                else
                {
                ...
                if (v.RelateImageFileName.IsNullorEmpty()==false)
                {
                    sb.Append("<img src = '" + helper.Content("~/UploadImage/VideoItem/" + v.RelateImageFileName) + "'  alt='' align='left'>");
                }
                else
                {
            ...
            videoitem = videoitem.Take(4).ToList();
            ...
            foreach (var v in videoitem)
            {
                ...
                if (v.Link_Mode == 1)
                {
                    if (menuid != "-1")
                    {
                        sb2.Append("<a href='" + helper.Action("VideoView", new { id = v.ItemID, mid = menuid, groupid = v.GroupID }) + "' title='" + v.Title + "'>");
                    }
                    else
                    {
                        sb2.Append("<a href='" + helper.Action("VideoView", new { id = v.ItemID, groupid = v.GroupID }) + "' title='" + v.Title + "'> ");
                    }
                }
                else
                {
                ...
                {
                    sb2.Append("<img src = '" + helper.Content("~/UploadImage/VideoItem/" + v.RelateImageFileName) + "'  alt='" + v.Title+"' align='left'>");
                }
                else
                {
